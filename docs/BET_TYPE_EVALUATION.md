...
CLIは両ファイルの改変検知とレースIDの一対一対応を確認し、券種別のMarkdown表を標準出力へ出す。`--report` を指定すると、レースID順に正規化した入力ファイルのSHA-256、構造化集計、全レース×6券種の決済台帳を、スキーマ版 `1.1` の `bet-types-evaluation.json` へ保存する。既存レポートは上書きしない。時刻や入力ディレクトリの絶対パスは保存しないため、同じ入力内容からは同じレポート内容を再生成できる。

`load_bet_type_evaluation_artifact` はレポート自体のSHA-256、入力順、レースIDの重複、全券種が全レースを1点ずつ含むこと、決済台帳から再計算した集計が保存集計と一致することを検証する。旧スキーマ `1.0` は読込可能だが、決済台帳を持たないため新規保存はできない。レースの指定順は集計結果と保存内容に影響しない。
...
スキーマ `1.1` の決済台帳を持つ2つのレポートは、同一レースを対に保ったまま再標本化できる。
...
券種ごとに的中率差、回収率差、最高払戻除外後回収率差、最高1件依存度差の点推定と区間を示す。改善確率は、反復内の候補−基準が正なら1、同値なら0.5として集計する参考値であり、p値や有意差判定ではない。通常のレース単位再標本化であり、同一開催日内の相関や6券種を同時に見る多重比較は補正しない。
