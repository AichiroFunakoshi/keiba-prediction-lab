...
監査済み予測バンドルとウォークフォワード成果物を`ReadOnlyAppSnapshot`へ変換する型付き境界は実装済みである。次はループバックだけで待ち受ける読み取り専用HTTP表示を追加する。ファイル選択、学習、予測、保存操作はまだ追加しない。視覚画面を作る前に、表示対象とユーザー操作の明確な案を固定する。
