...
監査済み予測バンドルとウォークフォワード成果物を`ReadOnlyAppSnapshot`へ変換する型付き境界と、IPv4ループバックだけで待ち受ける読み取り専用HTTP APIは実装済みである。次はこのAPIだけを読む視覚画面を追加し、実購入候補1点100円、影予測0円、勝率順位、評価指標を混同しないレイアウトを固定する。ファイル選択、学習、予測、保存操作はまだ追加しない。
