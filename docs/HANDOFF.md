...
監査済み予測バンドルとウォークフォワード成果物を`ReadOnlyAppSnapshot`へ変換する型付き境界、IPv4ループバックだけで待ち受ける読み取り専用HTTP API、同APIだけを読む視覚画面は実装済みである。実購入候補1点100円、影予測0円、勝率順位、評価指標は別領域に固定した。次は読み取り専用のまま、予測だけ・評価だけ・両方という欠落状態と小さい画面での表示を固める。ファイル選択、学習、予測、保存操作はまだ追加しない。
