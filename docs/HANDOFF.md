...
- 購入額はオッズで変更しない。独立予測は保持し、時刻固定したオッズを使う市場混合版は別成果物・別モデル版として扱う
...
このコマンドは限定したプロジェクト候補だけを探索し、予測と払戻の監査状況をJSONで返す。`ready_for_evaluation`となった候補だけを後続診断へ渡す。
...
発走直前の正式固定では`predict-race`、`predict-race-day`、`audit-race-inputs`に`--require-complete-body-weight`を指定できる。1頭でも当日馬体重が欠ける場合は保存前に拒否し、開催日一括予測では部分成果物を残さない。夜間の研究予測では省略可能であり、モデル順位や係数は変更しない。
...
2026年9月5日、時刻固定した単勝オッズを別版として参照する`build-market-blend`を追加した。モデル65%・市場35%の対数意見プールを使い、JRA取得時刻より後に発走するレースだけを保存する。元の発走前予測は変更しない。35%は結果データで最適化しておらず、固定ウォークフォワード比較前の暫定値である。RaceWeaveは同じ開催日マニフェストのSHA-256に結び付いた`market-blend.json`だけを表示へ重ねる。枠色は新規取得ではJRA出走表の`td.waku`から抽出し、旧成果物の表示では公式の馬番・頭数規則から補完する。

`train-model --calibration-races N`は、時間順で新しい末尾Nレースを係数学習から分離し、既存の温度スケーリングを本番モデル成果物へ保存する。旧スキーマ1.0モデルは引き続き読める。校正は順位を変更せず確率の過信だけを補正し、対象開催日の結果を校正へ混ぜない。

`--track-condition-v2`は、芝・ダート・障害を分離した同一馬場状態の勝率・3着内率・経験数を使う候補モデルである。旧モデルは読み込み互換を保ち、v2は別のモデル版として保存する。直近開催だけで昇格させず、同じ固定ウォークフォワード窓で従来版と比較する。早朝から馬場が変わった場合は特徴量の係数では解決できないため、発走前の再取得と未発走レースの別成果物への再予測を行う。詳細は[馬場適性特徴量 v2](TRACK_CONDITION_V2.md)を参照する。
...
実成果物がない新しいMacでも画面を確認できるよう、`init-ui-demo`は12レース分の合成入力、モデル、監査済み予測バンドル、ウォークフォワード成果物、開催日マニフェストと`race-day-provenance.json`を上書きせず生成する。生成直後のディレクトリは`audit-race-day`で開催日全体を再監査できる。`serve-ui-demo`は全成果物を再監査し、既定ブラウザで読み取り専用画面を開く。合成デモは`local/`だけに置き、実レースの精度主張や購入判断へ転用しない。実成果物を表示するときは`serve-read-only-api --open-browser`を使う。
...
近走の着順・同時騎乗成績・前走からの条件差を使う任意候補 `--recent-form-v4` を追加した。既定モデルは変更しない。特徴量定義・互換性・採用判断は[近走モデル候補](RECENT_FORM_V4.md)を参照する。

モデル単体の誤差と市場混合後の誤差は分けて評価する。ローカルの有効設定を予測生成とアプリで共用する経路、および同時比較版の表示は [混合予測設定](JOINT_PREDICTION_PROFILE.md) を参照。既存の低水準コマンドの既定比率は互換性のため保持するが、日々の運用は `predict-profile-day` を使う。

独自予測を主軸にする運用では、有効なローカル設定の `market_weight: 0` を使う。市場との比較は別設定・別表示にする。対戦相手と適性を追加した `--ability-v5`、根拠表示、欠測・未実装項目の境界は [独自能力モデルv5](INDEPENDENT_ABILITY_V5.md) を参照。

## 三連複主候補への移行

ユーザーの券種変更指定により、`selection_objective: trio` の日次運用とアプリ主表示は三連複1点100円へ移行する。従来の三連単限定記述は旧バンドルの互換契約として残す。新規の三連複固定ファイル、評価範囲、旧予測の扱いは [三連複主候補](TRIO_SELECTION.md) を参照。

三連複専用の条件付き集合モデルを比較候補として追加。固定期間検証、次回の0％・20％同時計算、アプリ上の開発検証表示は [三連複専用学習](TRIO_SET_LEARNING.md) を参照。既存の主予測は自動置換しない。
