...
- この申し送り直前の完了PR: `#33 Add local walk-forward evaluation reports`
...
### 優先1：ウォークフォワード成果物の再読込・監査

現在は成果物をSHA-256付きで保存できるが、保存済み`walk-forward.json`を読み込み、内部ハッシュ・型・集計整合性を再検証する専用CLIがない。モデル成果物や予測バンドルと同じ水準のローダーと`audit-walk-forward-report`を追加する。
