...
状態：`review_required`（取得器はあるが、利用許諾をプロジェクトとして保証しない）
...
- クロール表示：2026年9月3日確認時の`robots.txt`は全ユーザーエージェントに対して`Disallow`を指定していない
- 制限：オープンデータではない。取得物、整形CSV、学習モデルを公開・再配布せず、`local/`だけへ保存する

結論：`fetch-jra-web`は標準経路ではなく、利用者が最新条件を独立に確認した場合だけ使う実験経路とする。実行時は私的利用・再配布禁止への明示同意を必須とし、1秒未満の実アクセスを拒否し、取得上限、全URL・取得時刻・SHA-256の保存、途中失敗時の全体破棄を強制する。ページ構造変更時は推測で列を補わず停止する。
...
中央競馬について、無料かつ利用許諾が明確な標準データ経路は未確定である。JRA公開ページ取得器を使う場合も、取得物はローカルだけに保存し、GitHubへコミットしない。ページ構造、利用条件、`robots.txt`が変わった場合は取得を止めて再確認する。
