...
バージョン0.2はUI-1の読み取り専用機能を`.app`へ包装し、Finderから監査済みの開催日成果物を選択できる。学習、予測実行、結果取得、保存は行わず、既存CLIで生成された成果物の表示に限定する。
...
Finderで`dist/RaceWeave.app`を開く。ブラウザとTerminalを操作する必要はない。macOS標準のファイル選択画面で、開催日出力ディレクトリ内の`race-day.json`を選ぶ。
...
ファイル選択をキャンセルした場合だけ、初回に次のGit管理外領域へ合成デモを作る。
