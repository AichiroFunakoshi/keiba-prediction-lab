...
各取得先には、改変していないJV-DataレコードのJSONLと、取得時刻・要求条件・件数・SHA-256を含むマニフェストを新規作成する。既存出力は上書きしない。JV-Linkの`JVRead`は`-3`で0.25秒待機し、既定600秒を超えて連続した場合は停止する。`-1`は物理ファイル切替として読取りを継続し、`0`で取得完了、その他の負値では出力全体を除去して失敗する。待機上限は`--max-poll-seconds`で明示変更できる。
